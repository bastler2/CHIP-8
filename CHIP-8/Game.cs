    ...
    public class Game
    {
        ...
        public Game(string[] args)
        {
            ...
            if (args.Length > 0)
                ...
                cpu.loadRom(@"Space Invaders [David Winter].ch8");
        ...
        public void loadConfig()
        {
            if (!File.Exists("config.json"))
                File.WriteAllText("config.json", @"{""FpsRender"":60,""FpsTimer"":60,""ExecutionSpeed"":100,""KeyBinding"":[96,97,98,99,100,101,102,103,104,105,65,66,67,68,69,70],""ShowDebugging"":false}");
            var config = JsonSerializer.Deserialize<Config>(File.ReadAllText("config.json"));
            //cpu.RenderFps = config.FpsRender;
            cpu.TimerFps = config.FpsTimer;
            cpu.speed = config.ExecutionSpeed;
